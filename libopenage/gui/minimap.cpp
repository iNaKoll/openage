...
background_update get_background_update(GameMainLink *game, const std::vector<gamedata::palette_color> &palette) {
	...
	bg_update.background_tex = generate_minimap_background(*terrain, palette);
	bg_update.bounding_square = terrain_bounding_square(*terrain);
...
std::unique_ptr<QSGNode> Minimap::create_tree() {
	...
	auto material_ptr = std::unique_ptr<QSGSimpleMaterial<Icon>>(TexturedPointsShader::createMaterial());
	...
	this->update_backround = [this, background, world_to_background, turn] (background_update &&bg_update, const coord_data &engine_coord_data) {
		if (bg_update.is_fresh()) {
			...
			auto background_gui_tex = gui_texture_from_img_data(std::move(std::get<0>(bg_update.background_tex)), std::get<1>(bg_update.background_tex), std::get<2>(bg_update.background_tex));

			background->setTexture(std::get<std::unique_ptr<GuiTexture>>(background_gui_tex).release());
			background->setSourceRect(std::get<QRectF>(background_gui_tex));

			auto terrain_bounding_square_max_side_len = terrain_square_dimensions(bg_update.bounding_square);
			...
			auto terrain_bounding_rect_center_phys = terrain_square_center(bg_update.bounding_square).to_tile3().to_phys3();
...
QSGNode* Minimap::updatePaintNode(QSGNode *node, UpdatePaintNodeData*) {
	...
	if (this->game && this->game->get_state() != GameMainLink::State::Null) {
		if (root || this->hidden_root || this->bg_update.is_fresh()) {
			if (!root)
...
void Minimap::updatePolish() {
	emit this->game->in_game_logic_thread_blocking([this] {
		if (unwrap(this->game)->is_game_running()) {
			...
			if (this->game_dirty || this->need_freash_bg || this->last_bounding_rect != this->boundingRect() || this->terrain_usage_hash != new_terrain_usage_hash) {
